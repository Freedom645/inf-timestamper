...
実機検証前の版を別 PC へ配りたいときは、正式版と同じワークフローでプレリリースを作れる。

1. `<Version>` を **`1.2.0-alpha.1`** のように SemVer のプレリリース表記にする
   （`AssemblyVersion` / `FileVersion` は数値のみなので `1.2.0.0` のまま）
2. `docs/release-notes/v1.2.0-alpha.1.md` を書く
3. コミットして push する。**`main` でなくてよい**（feature ブランチのままで構わない）
4. Actions タブ → 「リリース（ドラフト作成）」 → Run workflow で、**そのブランチを選んで**実行する
5. ドラフトの中身を確認して **Publish release** を押す。選んだブランチの先頭コミットに
   `v1.2.0-alpha.1` タグが打たれる

ワークフローは `<Version>` に `-` が含まれていればプレリリースと判断し、`vpk upload github --pre` で
GitHub Release に pre-release の印を付ける。これにより:

- **正式版の利用者には見えない。** アプリのバージョンチェックは `releases/latest` を見ており、
  GitHub はここにプレリリースを含めない。Velopack 側も `GithubSource(prerelease: false)` で除外する
- **α 版の利用者は次の α 版と正式版の両方を拾う。** 実行中のバージョンがプレリリースなら、
  アプリは `releases` 一覧からプレリリース込みで最もバージョンの高いものを選ぶ
  （`SemanticVersion` で比較するので `1.2.0-alpha.2 > 1.2.0-alpha.1`、`1.2.0 > 1.2.0-alpha.2`）
- α 版の連番を進めるときは `<Version>` を `1.2.0-alpha.2` にして同じ手順を繰り返す。
  正式版にするときは `1.2.0` に戻して `main` から実行する

`docs/release-notes/` はバージョンごとに 1 ファイルなので、α 版のノートは正式版のノートとは
別に作る（正式版のノートに統合してよい）。
...
- タグは `v1.0.0` 形式（α 版は `v1.2.0-alpha.1`）。`VersionComparer` が `v` プレフィックスと
  SemVer のプレリリース表記を解釈するので、これでアプリ側のバージョンチェックが動く
