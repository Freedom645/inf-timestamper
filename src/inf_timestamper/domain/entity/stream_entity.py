...
class StreamSession(BaseModel):
    ...
    def resume_recording(self) -> "StreamSession":
        """記録を再開する"""
    ...
    def complete_recording(self) -> "StreamSession":
        """記録を完了する"""
    ...
    def get_timestamp_list(self, start_time: datetime | None = None) -> list[tuple[timedelta, Timestamp]]:
        """タイムスタンプのリストを取得する"""
        ...
        stamps: list[tuple[timedelta, Timestamp]] = []
        for timestamp in self.timestamps:
            stamps.append((timestamp.get_elapse(start_time), timestamp))
        return stamps

    def get_latest_timestamp(self) -> Timestamp | None:
        """最新のタイムスタンプを取得する"""
