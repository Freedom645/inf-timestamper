...
YouTube にログインしておくと、**配信中のライブの概要欄をタイムスタンプで自動更新**することもできます
（任意。設定手順は [docs/youtube.md](docs/youtube.md)）。
...
- **YouTube 連携タブ**（任意）: ログインして自動更新を有効にすると、記録中にライブの概要欄の末尾へタイムスタンプを書き込み続けます。
  OAuth クライアントの作成が必要です（[docs/youtube.md](docs/youtube.md)）
